...
check_seed() {
    ...
    if ! check_daemon; then
        ...
        log_info "Starting daemon..."
        cd "$INSTALL_DIR"
        ./src/junocashd -daemon -exportdir=/root
        sleep 15
    fi
    ...
    if [[ "$SEED_PHRASE" != *"error"* ]] && [ -n "$SEED_PHRASE" ]; then
        echo -e "${GREEN}=== SEED PHRASE (BIP39 Mnemonic) ===${NC}"
        echo "$SEED_PHRASE"
        echo ""
    ...
    # Show shielded addresses and keys
    echo -e "${CYAN}=== SHIELDED ADDRESSES ===${NC}"
    ADDRESSES=$(./src/junocash-cli z_listaddresses 2>/dev/null)
    echo "$ADDRESSES"
    echo ""
    
    # Try to get transparent addresses
    echo -e "${CYAN}=== TRANSPARENT ADDRESSES ===${NC}"
    TRANSPARENT=$(./src/junocash-cli listreceivedbyaddress 0 true 2>/dev/null | grep -o '"address": "[^"]*"' | cut -d'"' -f4)
    
    if [ -n "$TRANSPARENT" ]; then
        echo "$TRANSPARENT"
        echo ""
        
        # Offer to dump transparent private keys
        echo -e "${YELLOW}Do you want to export transparent private keys? (y/n): ${NC}"
        read -r EXPORT_T
        
        if [[ "$EXPORT_T" == "y" || "$EXPORT_T" == "Y" ]]; then
            echo ""
            log_info "Exporting transparent private keys..."
            echo ""
            
            TIMESTAMP=$(date +%Y%m%d_%H%M%S)
            EXPORT_FILE=~/.junocash/transparent_keys_${TIMESTAMP}.txt
            
            echo "# Transparent Private Keys - $(date)" > "$EXPORT_FILE"
            echo "# DO NOT SHARE THIS FILE" >> "$EXPORT_FILE"
            echo "" >> "$EXPORT_FILE"
            
            while IFS= read -r addr; do
                if [ -n "$addr" ]; then
                    echo "Address: $addr"
                    PRIVKEY=$(./src/junocash-cli dumpprivkey "$addr" 2>&1)
                    
                    if [[ "$PRIVKEY" != *"error"* ]]; then
                        echo -e "  Private Key: ${GREEN}$PRIVKEY${NC}"
                        echo "Address: $addr" >> "$EXPORT_FILE"
                        echo "Private Key: $PRIVKEY" >> "$EXPORT_FILE"
                        echo "" >> "$EXPORT_FILE"
                    else
                        echo "  Private Key: [Error retrieving]"
                    fi
                fi
            done <<< "$TRANSPARENT"
            ...
            echo ""
            log_success "Transparent keys saved to: $EXPORT_FILE"
        fi
    else
        echo "No transparent addresses found"
    fi
    echo ""
...
import_private_key() {
    ...
    echo -e "${CYAN}Select Key Type to Import:${NC}"
    echo ""
    echo "1. Shielded Private Key (secret-extended-key-main...)"
    echo "2. Transparent Private Key (WIF format)"
    echo "3. Import from Backup File (wallet backup)"
    echo "4. Cancel"
    echo ""
    echo -n "Select [1-4]: "
    read -r key_type
    ...
    case $key_type in
        ...
        2)
            # Import Transparent Key
            echo ""
            log_info "Import Transparent Private Key"
            echo ""
            log_warning "Note: Juno Cash uses shielded addresses by default."
            log_warning "Transparent keys are for legacy/interop only."
            echo ""
            echo "Enter your transparent private key:"
            echo "(Format: WIF - starts with 5, K, or L)"
            echo ""
            echo -n "Private Key: "
            read -r PRIVATE_KEY
            ...
            echo ""
            echo -n "Add label (optional, press Enter to skip): "
            read -r LABEL
            
            log_info "Importing transparent key..."
            
            if [ -z "$LABEL" ]; then
                RESULT=$(./src/junocash-cli importprivkey "$PRIVATE_KEY" 2>&1)
            else
                RESULT=$(./src/junocash-cli importprivkey "$PRIVATE_KEY" "$LABEL" 2>&1)
            fi
            
            if [[ "$RESULT" == *"error"* ]]; then
                log_error "Failed to import: $RESULT"
            else
                log_success "Transparent key imported successfully!"
                echo ""
                log_info "Getting address..."
                ./src/junocash-cli getaddressesbyaccount ""
            fi
            ;;
        ...
        3)
            ...
            echo ""
            echo "Enter backup filename (without path):"
            echo -n "Filename: "
            read -r BACKUP_FILE
...
export_private_key() {
    ...
    echo ""
    echo "1. Export Shielded Private Key (for specific address)"
    echo "2. Export All Keys (full wallet backup)"
    echo "3. Export Viewing Key (read-only, safe to share)"
    echo "4. Cancel"
    echo ""
    echo -n "Select [1-4]: "
    read -r export_type
